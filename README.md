...
| Email | Resend HTTP API (optional) — logs instead of sending when unconfigured |
...
- **Email notifications on transactions** — deposit, withdraw, transfer, and loan-disbursement all fire a notification email ([`app/services/email_service.py`](backend/app/services/email_service.py)), including ones executed by the scheduled-transfer background job. Sending is fire-and-forget and never blocks or fails the transaction that triggered it. With no `RESEND_API_KEY` set (the default — nothing to sign up for to see this work), it logs the would-be email instead of sending it, visible in `docker compose logs backend`; set the key to actually deliver via [Resend](https://resend.com)'s HTTP API (chosen over SMTP because Render's free web service blocks outbound SMTP ports — see Deployment below). *Sandbox caveat:* until you verify a sending domain with Resend, their sandbox only delivers to the email address on the Resend account itself, not arbitrary recipients.

Phase 2 is now complete. What's left from the original project brief is purely operational: actually clicking through the Render/Vercel deployment (configs are ready, see below) rather than any more application code.
